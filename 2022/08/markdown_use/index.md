...
{{< admonition abstract>}}
这是一篇 Markdown 的简易教程，本人在写博客的过程中，经常发现自己会忘记一些语法，因此专门写下这篇文章来记录一下，以便以后遇到问题时能够更快速地查阅。
{{< /admonition >}}
...
{{% admonition warning "注意" %}}
Hugo 的公式渲染有时候会出现 bug，若公式没有渲染请刷新页面重新查看。
{{% /admonition %}}

### 8.1 行内公式

$\partial_t v - \frac{1}{\rho}\partial_x \tau = 0$, 行内公式的语法为
...
{{% admonition warning "注意" %}}
LaTeX 的换行符为 \\，但是这里使用的是 \\\\，主要是因为 Markdown 中的两个 \ 才表示一个 \，因此在换行时需要使用4个 \。
{{% /admonition %}}
